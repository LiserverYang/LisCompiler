...
| 测试 | **1152 个 gtest 全绿**（运行时用例进程内 MCJIT 执行 + 按物理核分片，全量约 18 s）；13 个 Examples 输出为回归基线（borrow 55/iterator 23/match 8/result 42/…） |
...
- Tests：1152 个 gtest（词法/语法/借用/运行时端到端），测试源分 4 个 TU 并行编译、测试按物理核分片并行跑
