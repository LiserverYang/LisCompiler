...
**另一个例外（E3017，对应 Rust 的 E0507）**：非 Copy 字段也不能**穿过引用**移出
——借用不拥有任何东西，搬走的值会同时被接收者和被借用者释放（实测：`let x = r.s;`
（`r: &mut P`、`s: String`）在退出时堆损坏 0xC0000374）。读 Copy 字段（`let n = r.n;`）
是复制不是移动；移动**引用本身**（`let q = r;`，`&mut T` 非 Copy）依然合法。
