...
- 可提升的成员包括**模块级 `let`**（常量）：`impt code { OP_ADD };` 之后 `OP_ADD` 裸名可用，
  也可以照常写 `code::OP_ADD`（2026-09-18 修：此前全局变量只被登记在 pass 2，选择性导入
  的检查跑在前面，于是恒报「module 'code' has no member 'OP_ADD'」）。
  被提升的**值**名只进符号表；只有**类型**名（struct/enum/trait）会进解析器的类型集合,
  因此 `if x == K { ... }` 里的 `{` 仍然是语句块，而不是结构体字面量。
...
{
    pub x: i32,
    y: i32        // 私有字段:仅本类型自己的方法内可访问（静态方法也算）
}
```

- 字段逗号分隔，允许尾逗号。
- **`pub` 只能用于结构体字段**；函数/方法/变体无可见性修饰。
- 字段可见性由编译器**强制**：私有字段在本类型的方法之外既不能读、也不能用于构造
  （E3015）。这样类型才能守住自己的不变量（例如 `String` 的 `data`/`len`/`cap`）。
