...
class FileIO:
    ...
    def QueryPermissions(self) -> int:
        ...
        self.BasePermissions = (
            Functions.AddPermissions(BasePermissionsTemp, FilePermissionsEnum.Read)
            if os.access(self.FilePathStr, os.R_OK)
            else self.BasePermissions
        )
        self.BasePermissions = (
            Functions.AddPermissions(BasePermissionsTemp, FilePermissionsEnum.Write)
            if os.access(self.FilePathStr, os.W_OK)
            else self.BasePermissions
        )
        self.BasePermissions = (
            Functions.AddPermissions(BasePermissionsTemp, FilePermissionsEnum.Run)
            if os.access(self.FilePathStr, os.X_OK)
            else self.BasePermissions
        )
    ...
    def GetSubFiles(self) -> list[str]:
        ...
        return [
            os.path.join(self.FilePathStr, file)
