...
public:
    ...
    std::shared_ptr<FunctionType> getFunction(std::string name, std::vector<FunctionType::Param> params, std::shared_ptr<Type> returnType)
    {
        ...
        auto functionType = std::make_shared<FunctionType>(name, params, returnType);
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<FunctionType>> functions;
    std::unordered_map<std::pair<void *, bool>, std::shared_ptr<ReferenceType>, PairHash> refCache;
};